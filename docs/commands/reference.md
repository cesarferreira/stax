...
- **Interactive Sync plan** — after fetch, stax refreshes PR metadata, then prints one **Sync plan** (trunk moves, merged/upstream-gone branches with PR numbers when known, optional `--restack` preview) and asks how to proceed. When deletions are listed: continue and delete all (non-blocking), per-branch prompts, or cancel (`Aborted.`, no undo receipt). When only trunk/restack changes are pending: **Continue sync** or **Cancel sync** (no bulk-delete wording). Skipped when `--quiet`, `--json`, `--force`, or when there is nothing to confirm.
