...
- **Interactive Sync plan** — after fetch, when there are merged/upstream-gone deletions and/or a `--restack` preview, stax prints one **Sync plan** and asks how to proceed: continue and delete all listed branches (non-blocking cases only), choose per branch (existing confirm/skip/worktree prompts), or cancel (prints `Aborted.`, restores any auto-stash, no undo receipt). Skipped when `--quiet`, `--json`, `--force`, or when there is nothing to confirm.
