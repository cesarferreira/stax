...
fn print_inbox(inbox: &ReviewInbox, plain: bool) {
    ...
    for pr in &inbox.pull_requests {
        ...
        for comment in &pr.comments {
            let marker = if comment.kind == "review" {
                "◆"
            } else {
                "●"
            };
            if let Some(location) = inbox_comment_location(comment) {
                println!(
                    "  {} {} {} {}",
                    marker,
                    comment.author.cyan(),
...
fn inbox_comment_location(comment: &InboxComment) -> Option<String> {
    let path = comment.path.as_ref()?;
    Some(match (comment.start_line, comment.line) {
