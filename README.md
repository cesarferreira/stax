...
| `st reviews --stack [--json]` | Stack-wide review/comment inbox (`st comments` remains the current-PR view) |
...
| `st run --parallel --jobs 4 <cmd>` | Run checks concurrently in isolated temporary worktrees without switching the main worktree |
| `st freeze` / `st unfreeze` | Protect/unprotect a tracked branch from history-rewriting restacks |
